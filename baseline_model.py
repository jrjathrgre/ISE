...
def main():
    ...
    # systems = ['batlik']
    num_repeats = 5  # Modify this value to change the number of repetitions
    ...
    baseline_results = pd.DataFrame(columns=['system', 'dataset', 'repeat', 'MAPE', 'MAE', 'RMSE'])

    for current_system in systems:
        datasets_location = 'datasets/{}'.format(current_system)  # Modify this to specify the location of the datasets

        csv_files = [f for f in os.listdir(datasets_location) if f.endswith('.csv')]  # List all CSV files in the
        # directory

        for csv_file in csv_files:
            print('\n> System: {}, Dataset: {}, Training data fraction: {}, Number of repeats: {}'.format(current_system,
                                                                                                         csv_file,
                                                                                                         train_frac,
                                                                                                         num_repeats))
